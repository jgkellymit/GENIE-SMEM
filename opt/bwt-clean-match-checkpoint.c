...
#pragma omp parallel num_threads(numthreads)
  {
    ...
    for(ii = 0; ii < batch_size; ii++) {
      ...
      int64_t ind = my_enc_qdb[rid * readlength + readlength - 1];
      m.k = count[ind]; // TODO
      m.l = count[ind + 1]; // TODO
      m.rid = rid;
      m.cur_ind = readlength - 2; // TODO
      metadata[ii] = m;
      // printf("...BATCH %lld: k %d, l %d, rid %d, cur_ind %d\n", ii, m.k, m.l, m.rid, m.cur_ind);
    }
    ...
    while(num_active > 0) {
      ...
      for(ii = 0; ii < num_active; ii++) {
        ...
        if((ep <= sp) || (metadata[ii].cur_ind < 0))
        {
          ...
          {
            BatchMetadata m;
            int64_t ind = my_enc_qdb[next_read * readlength + readlength - 1];
            m.k = count[ind]; // TODO
            m.l = count[ind + 1]; // TODO
            m.rid = next_read;
            m.cur_ind = readlength - 2; // TODO
...
int main(int argc, char **argv) {
  ...
  if(argc != 8)
  {
    printf("Need seven arguments : indexfile query_set referencesequence.fa batch_size readlength z n_threads\n");
    return 1;
  }
  uint8_t *reference_seq = NULL;
  int64_t ref_seq_len;
